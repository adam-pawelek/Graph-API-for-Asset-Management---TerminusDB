...
@router.post("/")
async def create_sensor(item : schema.SensorSchema, space_id:  Union[str, None] = None, current_user: models.User = Security(get_current_active_user, scopes=["admin"])):
...
@router.put("/")
async def update_sensor(sensor: schema.SensorSchemaUpdate,  id: Union[str, None] = None, current_user: models.User = Security(get_current_active_user, scopes=["admin"])):
    return sensor_crud.update_sensor(sensor,id)



@router.put("/change/sensor_location")
async def change_use_case(main_id: Union[str, None] = None,change_id: Union[str, None] = None, current_user: models.User = Security(get_current_active_user, scopes=["admin"]) ):
